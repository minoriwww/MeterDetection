...
def run_network(model=None, data=None, data_file='df_dh.csv', isload_model=False, testonly=False):
    ...
    try:
        ...
        hist = model.fit(
            ...
            # batch_size=512,
            nb_epoch=epochs, validation_data=(X_val, Y_val), callbacks=[early_stop],
            shuffle=False
...
def run_regressor(model=LSTM2, sequence_length = SEQ_LENGTH, data=None, data_file='df_dh.csv', isload_model=True, testonly=False):
    ...
    ################
    hist = estimator.fit(X_train, y_train, validation_data=(X_val, Y_val), callbacks=[checkpoint], epochs=epochs,
...
if __name__ == '__main__':
    ...
    plt.plot(x, mses, lw=1, color='r')

    axes.grid()
    plt.xlabel('Sequence Length')
    plt.ylabel('Mean Squared Error')
    fig.savefig(RESULT_PATH + str(prefix) + 'mses.png', dpi=300)
    np.save(RESULT_PATH + str(prefix) + 'mses.npy', np.asarray(mses))
